#FIX FIND_BEST_IOU_BOXES AND FORWARD
...
class YoloLoss(nn.Module):
    ...
    def find_best_iou_boxes(self, pred_box_list, box_target):
        ...
        best_ious = []
        best_boxes = []
        target = self.xywh2xyxy(box_target)
        for box in pred_box_list:
            ious = compute_iou(self.xywh2xyxy(box[:, :4]), target)
            best_ious.append(ious[torch.argmax(ious)])
            best_boxes.append(box[torch.argmax(ious)])
    ...
    def forward(self, pred_tensor, target_boxes, target_cls, has_object_map):
        ...
        # compute final loss
        final_loss = reg_loss + noobj_loss + contain_object_loss + class_loss
